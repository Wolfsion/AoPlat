    ...
    <template #optional="{ record }">
      <a-space>
        <a-button
          ...
          status="success"
          @click="reviewApp(record, REVIEW_STATUS_ENUM.PASS, '')"
        >
          通过
        </a-button>
        <a-button
          ...
          status="warning"
          @click="
            reviewApp(record, REVIEW_STATUS_ENUM.REJECT, '不符合上架要求')
          "
        >
          拒绝
        </a-button>
        <a-button status="danger" @click="deleteApp(record)">删除</a-button>
      </a-space>
    </template>
  </a-table>
</template>
...
import { reviewAppUsingPost } from "@/api/appController";
 ...
 * @param record
 */
const deleteApp = async (record: API.App) => {
